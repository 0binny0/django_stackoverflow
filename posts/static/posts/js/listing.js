...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  const sort_query = url_query_string.get("tab");
  debugger;
  const query_types = query_buttons.map((query_button) => query_button.textContent);
  if (!sort_query || !query_types.includes(sort_query)) {
    ...
    const button_index = query_buttons.findIndex(
      (query_button) => {
        return query_button.textContent === sort_query;
      }
    );
